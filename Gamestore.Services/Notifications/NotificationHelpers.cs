...
internal static class NotificationHelpers
{
    ...
    internal static async Task NotifyUserOrderStatusChanged(Order? order, string orderStatus, INotificationService notificationService, IHttpContextAccessor httpContextAccessor, UserManager<AppUser> userManager)
    {
        ...
        if (user != null)
        {
            ...
            if (usr != null)
            {
                ...
                var message = new NotificationMessage() { To = usr.Email!, Subject = $"Order no. {order.Id} received new status", Body = $"New order status: {orderStatus}", NotificationMethods = methods.ToList() };
