...
public class GameServiceTests
{
    ...
    public async Task AddGameAsyncShouldAddGame()
    {
        ...
        var gameToAdd = new GameModelDto()
        {
            ...
            GamePlatforms = BllHelpers.PlatformModelDtos,
            GameGenres = BllHelpers.GenreModelDtos,
        };
        ...
        && x.Key == gameToAdd.Key && x.Description == gameToAdd.Description && x.PublisherId == gameToAdd.PublisherId
        && x.Price == gameToAdd.Price && x.UnitInStock == gameToAdd.UnitInStock && x.Discount == gameToAdd.Discount)));
    ...
    public async Task UpdateGameAsyncShouldUpdateGame()
    {
        ...
        gameToUpdate.Discount = 75;
        gameToUpdate.UnitInStock = 10;
        gameToUpdate.PublisherId = Guid.NewGuid();
        ...
        unitOfWork.Verify(x => x.GameRepository.UpdateAsync(It.Is<Game>(x => x.Id == gameToUpdate.Id && x.Name == gameToUpdate.Name
            && x.Key == gameToUpdate.Key && x.Description == gameToUpdate.Description && x.PublisherId == gameToUpdate.PublisherId
            && x.Price == gameToUpdate.Price && x.UnitInStock == gameToUpdate.UnitInStock && x.Discount == gameToUpdate.Discount)));
